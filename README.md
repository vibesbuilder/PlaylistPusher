...
3. Fix wrong matches per track: pick an alternative, search again, paste a Spotify link, listen, or skip
...
3. **Find tracks** – results appear while the search is running. The target playlist can still be changed at the top of the review screen.
...
6. **Import** – shows a summary; tracks are added only after you confirm
...
| With time, date or number in front | `14:03:22 Falco – Rock Me Amadeus`, `12. Wanda - Bologna` |
...
- Access tokens and your review progress are stored only in your browser (localStorage of `http://127.0.0.1:8138`) and sent only to Spotify.
...
web/js/review.js       review screen
web/js/match.js        search strategy and scoring
web/js/parse.js        list parser
web/js/spotify.js      Spotify Web API client (PKCE, retries, rate limits)
